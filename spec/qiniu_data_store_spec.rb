...
require 'yaml'
require 'dragonfly/qiniu_data_store'

describe Dragonfly::QiniuDataStore do

  # To run these tests, put a file ".qiniu_spec.yml" in the dragonfly root dir, like this:
  # access_key: XXX
  # secret_key: XXX
  # bucket_name: XXX
  # url_host: XXX
  # key_prefix: XXX
  if File.exist?(file = File.expand_path('../qiniu_spec.yml', __FILE__))
  ...
  if enabled
    ...
    before(:each) do
      @data_store = Dragonfly::QiniuDataStore.new(
  ...
  describe "registering with a symbol" do
    it "registers a symbol for configuring" do
      ...
      app.datastore.should be_a(Dragonfly::QiniuDataStore)
    end
  end
  ...
  describe "not configuring stuff properly" do
    it "should require a bucket name on write" do
      ...
      proc{ @data_store.write(content) }.should raise_error(Dragonfly::QiniuDataStore::NotConfigured)
    end

    it "should require an access_key on write" do
      @data_store.access_key = nil
      proc{ @data_store.write(content) }.should raise_error(Dragonfly::QiniuDataStore::NotConfigured)
    end

    it "should require a secret key on write" do
      @data_store.secret_key = nil
      proc{ @data_store.write(content) }.should raise_error(Dragonfly::QiniuDataStore::NotConfigured)
    end

    it "should require a bucket name on read" do
      @data_store.bucket_name = nil
      proc{ @data_store.read('asdf') }.should raise_error(Dragonfly::QiniuDataStore::NotConfigured)
    end

    it "should require an access_key on read" do
      @data_store.access_key = nil
      proc{ @data_store.read('asdf') }.should raise_error(Dragonfly::QiniuDataStore::NotConfigured)
    end
  ...
  describe "urls for serving directly" do
    ...
    it "should use path style if the bucket is not a valid Qiniu subdomain" do
